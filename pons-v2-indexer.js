...
(function () {
  ...
  // Every eth_call/eth_getLogs this file makes goes through this single
  // lock, so only 1 request is ever in flight at a time from this file.
  // pons.js is already firing its own parallel batch of calls for token
  // metadata, and the RPC proxy apparently has a tight per-second limit
  // (observed 429s even on a single isolated getLaunchedToken call) —
  // so this file no longer adds to that burst. Slower, but it actually
  // completes instead of failing outright.
  let rpcLock = Promise.resolve();

  function withRpcLock(fn) {
    const previous = rpcLock;
    let release;
    rpcLock = new Promise(resolve => { release = resolve; });
    return previous.then(fn).finally(release);
  }

  async function rpc(method, params = []) {
    return withRpcLock(async () => {
      const url = getRpcUrl();
      ...
      for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        ...
        if (response.status === 429 && attempt < MAX_RETRIES) {
          ...
          const delay = Number.isFinite(retryAfterHeader) && retryAfterHeader > 0
            ...
            : 600 * Math.pow(2, attempt) + Math.random() * 250;
          console.warn(`[PonslyV2] [${method}] 429 rate limited, retrying in ${Math.round(delay)}ms (attempt ${attempt + 1}/${MAX_RETRIES})`);
          await sleep(delay);
          continue;
        }
        ...
        // Small courtesy gap even after a successful call, so a burst
        // of sequential rpc() calls in a loop doesn't itself trigger
        // the next 429. Held under the lock so the next queued call
        // waits for it too.
        await sleep(80);
        return json.result;
      }

      throw new Error(`[${method}] RPC failed after ${MAX_RETRIES} retries (rate limited)`);
    });
  }
  ...
  async function getRecentTrades(tokenAddress, options = {}) {
    ...
    const runner = (async () => {
      ...
      // NOTE: this used to also try getLaunchBlock() — an exact-launch-block
      // lookup via a single unchunked eth_getLogs from block 0 to head — for
      // any token with no cached block yet. That's an unbounded full-chain
      // history scan in one request, which is exactly the kind of query
      // public RPC endpoints throttle hardest. It was firing on every
      // never-before-indexed token (which is most of them, on a fresh
      // Supabase table), which is why 429s kept happening regardless of
      // the retry/serialization fixes — those fixes make single requests
      // more polite, they don't make one enormous request smaller.
      // A bounded lookback is precise enough for "recent trades" anyway.
      if (fromBlock == null) fromBlock = Math.max(0, head - 50000); // bounded first-index window, not a fabricated trade
