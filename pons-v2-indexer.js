...
(function () {
  ...
  /*
   * keccak256 over a hex string, via a small pure-JS implementation
   * (no external library — this static site loads no bundler/CDN
   * crypto lib beyond supabase-js). Only used for the one poolId
   * computation above.
   */
  ...
  // Minimal, dependency-free Keccak-256 (NIST SHA3 predecessor —
  // note this is *not* the same as standardized SHA3-256; Ethereum
  // uses the original Keccak padding). Adapted for a single one-shot
  // hash call, not a streaming API, since that's all this file needs.
  function keccak256(message) {
    const RC = [
      1n, 32898n, 9223372036854808714n, 9223372039002292224n, 32907n, 2147483649n,
      9223372039002292481n, 9223372036854808585n, 138n, 136n, 2147516425n, 2147483658n,
      2147516555n, 9223372036854775947n, 9223372036854808713n, 9223372036854808579n,
      9223372036854808578n, 9223372036854775936n, 32778n, 9223372039002259466n,
    ...
    const blockCount = Math.floor(message.length / rate) + 1;
    ...
    function rotl64(x, n) {
      ...
      return n === 0n ? x : ((x << n) | (x >> (64n - n))) & MASK;
    }
    ...
    for (let off = 0; off < padded.length; off += rate) {
      ...
      for (let round = 0; round < 24; round++) {
        ...
        for (let x = 0; x < 5; x++) {
          for (let y = 0; y < 5; y++) {
            B[y * 5 + ((2 * x + 3 * y) % 5)] = rotl64(s[x + y * 5], ROT[x][y]);
          }
        }
        ...
        for (let x = 0; x < 5; x++) {
          for (let y = 0; y < 5; y++) {
            const MASK = (1n << 64n) - 1n;
            s[x + y * 5] = (B[x + y * 5] ^ (~B[((x + 1) % 5) + y * 5] & B[((x + 2) % 5) + y * 5])) & MASK;
          }
        }
