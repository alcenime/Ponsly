...
(function () {
  ...
  // TEMP DEBUG: tracks the most recent RPC failure so it can be
  // surfaced in the UI while we diagnose why metadata reads are
  // coming back empty. Remove once this is resolved.
  window.__ponslyLastRpcError = null;

  const RPC_MAX_RETRIES = 3;
  const RPC_RETRY_BASE_DELAY_MS = 400;

  // Runs `fn` over `items` with at most `limit` in flight at once —
  // keeps us from firing dozens of parallel eth_call batches (each
  // token read is already 8 parallel calls on its own) and tripping
  ...
  async function rpc(method, params = []) {
    ...
    for (let attempt = 0; attempt <= RPC_MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        const message = `[${method}] fetch failed: ${error && error.message ? error.message : error}`;
        window.__ponslyLastRpcError = message;
        throw error;
      }

      // Rate limited — back off and retry instead of failing the
      // whole read immediately. Public RPC endpoints throttle bursts
      // of parallel eth_call requests, which this app sends a lot of.
      if (response.status === 429 && attempt < RPC_MAX_RETRIES) {
        ...
        const delay = Number.isFinite(retryAfterHeader) && retryAfterHeader > 0
          ...
          : RPC_RETRY_BASE_DELAY_MS * Math.pow(2, attempt) + Math.random() * 200;
        await sleep(delay);
        continue;
      }

      if (!response.ok) {
        const message = `[${method}] RPC HTTP ${response.status}`;
        window.__ponslyLastRpcError = message;
      ...
      if (json.error) {
        const message = `[${method}] ${json.error.message || `RPC error ${json.error.code || ""}`}`;
        window.__ponslyLastRpcError = message;
        throw new Error(message);
      }
