...
(function () {
  ...
  function buildWidgetUrl(poolAddress, options) {
    const theme = options.theme === "light" ? "pe-light" : "pe-dark";

    const url = new URL(
      `${DEXTOOLS_WIDGET_BASE}/${encodeURIComponent(DEXTOOLS_CHAIN_SLUG)}/${theme}/${poolAddress}`
    );

    url.searchParams.set("theme", options.theme === "light" ? "light" : "dark");
    url.searchParams.set("chartType", "2"); // candles
    url.searchParams.set("chartResolution", String(options.chartResolution || 30));
  ...
  function mountChart(tokenAddress, container, options = {}) {
    ...
    const url = buildWidgetUrl(poolAddress, options);
